...
Paperclip generates a small adapter-managed `mcp.json` and passes it to Claude Code with `--mcp-config`. By default it contains:
...
The adapter does not pass `--strict-mcp-config`, so existing user or project MCP config can still load. Set `PAPERCLIP_APPROVED_MCP_SERVERS=none` (also `false` or `off`) to disable the generated config, or set a comma list such as `PAPERCLIP_APPROVED_MCP_SERVERS=playwright` to narrow it. Unknown names are skipped with a run-log warning.
