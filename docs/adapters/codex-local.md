...
Paperclip exposes approved MCP servers through managed Codex config. By default it writes a managed `[mcp_servers.playwright]` block to the effective `CODEX_HOME/config.toml` for the duration of each run:
...
Set `PAPERCLIP_APPROVED_MCP_SERVERS=none` (also `false` or `off`) to disable this sync, or set a comma list such as `PAPERCLIP_APPROVED_MCP_SERVERS=playwright` to narrow it. Unknown names are skipped with a run-log warning. If adapter config explicitly sets `env.CODEX_HOME`, Paperclip treats that home as user-managed and does not mutate it.
