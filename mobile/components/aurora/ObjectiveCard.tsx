...
export interface ObjectiveCardProps {
  ...
  value: string;
  context?: string;
  metrics?: { value: string; label: string; align?: "left" | "right" }[];
  trailing?: ReactNode;
  big?: boolean;
}
...
export function ObjectiveCard({
  ...
  return (
    <View>
      {big ? <Animated.View style={[styles.glow, glowStyle]} pointerEvents="none" /> : null}
      <GlassCard radius={20} padding={18}>
        ...
        {metrics?.length ? (
          <View style={styles.metrics}>
            {metrics.slice(0, 2).map((metric, i) => (
              <View
                ...
                style={[
                  styles.metric,
                  metric.align === "right" ? styles.metricRight : null,
                ]}
              >
                <Text style={text.displayMd} numberOfLines={1}>
                  {metric.value}
                </Text>
          ...
          <Text style={[text.small, { marginTop: 10 }]}>{context}</Text>
        ) : null}
      </GlassCard>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  labelRow: { flexDirection: "row", alignItems: "center", gap: 7, flex: 1, minWidth: 0 },
