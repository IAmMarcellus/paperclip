...
export const api = {
  ...
  // --- oversight voice (Mergatroid) -------------------------------------
  // Which stack serves the call — mirrors the server's VOICE_PROVIDER env.
  oversightVoiceConfig: () =>
    apiFetch<{ provider: VoiceProvider }>("/board/oversight/voice/config"),
  // Server-side call setup, gated by enableVoiceChat + local_trusted + instance admin (see
  // server/src/routes/board-chat.ts). This build speaks only the self-hosted pipecat path
  // (SmallWebRTC to the local sidecar) — `providers: ["pipecat"]` advertises that capability so the
  // server returns `{provider:"pipecat", offerPath}` instead of the legacy ElevenLabs token that
  // `connectionType:"webrtc"` alone means to old builds. If the server is configured for
  // ElevenLabs, the response's `provider` says so and the call screen explains the mismatch.
  oversightVoiceToken: () =>
    apiFetch<OversightVoiceToken>("/board/oversight/voice/token", {
      method: "POST",
      body: { connectionType: "webrtc", providers: ["pipecat"] },
    }),
