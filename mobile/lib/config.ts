...
// Build-time backend the app auto-connects to at launch (there is no in-app Connect screen).
// Committed defaults live in `.env` (localhost); override per-machine in a gitignored `.env.local`
// to point a build at a specific box — e.g. this dev box over Tailscale
// (http://<host>.<tailnet>.ts.net:3100) — without committing the setup-specific value.
export const DEFAULT_BASE_URL =
  process.env.EXPO_PUBLIC_API_BASE_URL?.trim() || "http://localhost:3100";

// Optional bearer token sent with the auto-connect. Empty for local_trusted boards (no auth).
export const DEFAULT_TOKEN = process.env.EXPO_PUBLIC_API_TOKEN?.trim() || null;
