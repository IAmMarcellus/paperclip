...
describe("claude remote execution", () => {
  ...
  it("passes a generated MCP config for local execution", async () => {
    ...
    const mcpConfigArgIndex = call?.[2].indexOf("--mcp-config") ?? -1;
    expect(mcpConfigArgIndex).toBeGreaterThanOrEqual(0);
    const mcpConfigPath = call?.[2][mcpConfigArgIndex + 1];
    expect(mcpConfigPath).toContain(rootDir);
    expect(mcpConfigPath).toContain(path.join("companies", "company-1", "claude-prompt-cache"));
    expect(JSON.parse(await readFile(mcpConfigPath!, "utf8"))).toEqual({
      mcpServers: {
        playwright: {
          command: "npx",
    ...
    expect(call?.[2]).not.toContain("--strict-mcp-config");
  });
  ...
  it("prepares the workspace, syncs Claude runtime assets, and restores workspace changes for remote SSH execution", async () => {
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.paperclip-runtime/claude/mcp`,
      followSymlinks: true,
    }));
    ...
    expect(call?.[2]).toContain("--mcp-config");
    expect(call?.[2]).toContain(`${managedRemoteWorkspace}/.paperclip-runtime/claude/mcp/mcp.json`);
    expect(call?.[2]).not.toContain("--strict-mcp-config");
