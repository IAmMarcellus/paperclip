...
import { prepareClaudeMcpConfig } from "./mcp-config.js";
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configEnvStrings = Object.fromEntries(
  ...
  const mcpConfig = await prepareClaudeMcpConfig({
    env: configEnvStrings,
    rootDir: promptBundle.rootDir,
  });
  for (const note of mcpConfig.notes) {
    await onLog(
      note.includes("PAPERCLIP_APPROVED_MCP_SERVERS") ? "stderr" : "stdout",
      `[paperclip] ${note}\n`,
    );
  }
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    if (effectiveMcpConfigPath) {
      commandNotes.push(
        `Using Paperclip-approved Claude MCP config ${effectiveMcpConfigPath}.`,
      );
    }
    if (onMeta) {
      await onMeta({
