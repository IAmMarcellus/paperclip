...
/**
 * Classify an agent's configured CODEX_HOME to decide which config.toml
 * preparation steps apply.
 *
 * Two orthogonal concerns:
 *  - `userManagedCodexHome` — a home whose config.toml the provider /
 *    approved-MCP merge (`prepareCodexRuntimeConfig`) must leave untouched. Only
 *    an explicit CODEX_HOME that is NOT the per-agent home Paperclip persists for
 *    codex_local isolation qualifies.
 *  - `mcpEnvInjectionHome` — a home whose `[mcp_servers.*.env]` tables need
 *    adapterConfig.env injected into them. codex sanitizes an MCP child's
 *    environment to a fixed allowlist and never forwards the parent process env,
 *    so secrets/knobs in adapterConfig.env reach an MCP server ONLY via its env
 *    table. This is required for ANY explicit CODEX_HOME — including the
 *    per-agent managed home (which equals `managedAgentHome`); the default shared
 *    managed home (no explicit CODEX_HOME) is handled by prepareManagedCodexHome
 *    and is intentionally excluded.
 *
 * Pure/synchronous so the classification is unit-testable in isolation. Guarding
 * MCP-env injection on `userManagedCodexHome` (rather than any configured home)
 * silently dropped injection for per-agent managed homes — the regression this
 * helper locks against.
 */
export function classifyConfiguredCodexHome(input: {
  configuredCodexHome: string | null;
  managedAgentHome: string;
}): {
  isPaperclipManaged: boolean;
  userManagedCodexHome: string | null;
  mcpEnvInjectionHome: string | null;
} {
  const { configuredCodexHome, managedAgentHome } = input;
  const isPaperclipManaged =
    configuredCodexHome !== null &&
    path.resolve(configuredCodexHome) === path.resolve(managedAgentHome);
  return {
    isPaperclipManaged,
    userManagedCodexHome:
      configuredCodexHome && !isPaperclipManaged ? configuredCodexHome : null,
    mcpEnvInjectionHome: configuredCodexHome,
  };
}
