...
import { prepareCodexRuntimeConfig, prepareCodexMcpEnvInjection, CODEX_RESERVED_ENV_KEYS } from "./runtime-config.js";
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const { userManagedCodexHome, mcpEnvInjectionHome } = classifyConfiguredCodexHome({
    configuredCodexHome,
    managedAgentHome: resolveManagedCodexAgentHomeDir(process.env, agent.companyId, agent.id),
  });
  ...
  // Merge custom model providers (PAPERCLIP_CODEX_PROVIDERS) and approved MCP
  // servers into Paperclip-managed CODEX_HOME config before shipping it to a
  // remote execution target. Arbitrary env.CODEX_HOME overrides remain
  // user-managed and are skipped, but the per-agent CODEX_HOME that Paperclip
  // itself persists for codex_local isolation is still a managed home.
  ...
  const preparedRuntimeConfig = await prepareCodexRuntimeConfig({
    ...
    codexHome: userManagedCodexHome ? null : effectiveCodexHome,
  });
  // codex sanitizes the MCP-server environment to a fixed allowlist, so secrets/knobs in
  // adapterConfig.env never reach an MCP child via the process env. Inject adapterConfig.env
  // into the existing [mcp_servers.*.env] tables for ANY explicit CODEX_HOME so MCP servers
  // receive them — this includes the per-agent home Paperclip persists for codex_local
  // isolation (whose config.toml prepareCodexRuntimeConfig otherwise leaves untouched when no
  // providers/approved-MCP are configured). The injection backs up + restores the file after
  // the run and is idempotent, so it is safe to run alongside prepareCodexRuntimeConfig.
  const preparedMcpEnv = mcpEnvInjectionHome
    ? await prepareCodexMcpEnvInjection({ codexHome: mcpEnvInjectionHome, env: envConfigStrings })
    : { notes: [] as string[], cleanup: async () => {} };
  try {
    for (const note of [...preparedRuntimeConfig.notes, ...preparedMcpEnv.notes]) {
    ...
    // Propagate the agent's adapterConfig.env into the spawned codex process env, as a base
    // layer that the Paperclip-computed assignments below (CODEX_HOME, PAPERCLIP_*, ...) still
    // override. This mirrors what the claude_local adapter does and lets the agent's own tools
    // see its configured secrets/knobs. NOTE: codex does NOT forward this process env to MCP
    // server children (it sanitizes MCP env to a fixed allowlist), so MCP servers are handled
    // separately via prepareCodexMcpEnvInjection below. CODEX_RESERVED_ENV_KEYS are excluded so
    // we never clobber the real (possibly remote) CODEX_HOME, never leak OPENAI_API_KEY (which
    // would flip codex from ChatGPT-subscription auth to API-key billing), and never override the
    // adapter-minted PAPERCLIP_API_KEY.
    for (const [key, value] of Object.entries(envConfigStrings)) {
      if (!CODEX_RESERVED_ENV_KEYS.has(key)) env[key] = value;
    }
    ...
    if (preparedRuntimeConfig.notes.length > 0 || preparedMcpEnv.notes.length > 0) {
      commandNotes.unshift(...preparedRuntimeConfig.notes, ...preparedMcpEnv.notes);
    }
    ...
    // Restore the config.toml(s) we prepared so provider-routing / injected
    // secrets never leak into the next run. This finally starts the moment
    // prepareCodexRuntimeConfig returns, so a throw anywhere in the remaining
    // setup (skill injection, remote runtime preparation, command building)
    // restores the original too. If the process dies before reaching this, each
    // step's next prepare restores the original from its own pre-run backup file.
    //
    // Unwind in reverse prepare order (LIFO): mcp-env injection was prepared
    // last, so restore it first. On a per-agent managed home both steps can
    // touch the same config.toml (each with its own distinct backup file), and
    // reverse order guarantees the true pre-run original is what survives.
    await preparedMcpEnv.cleanup();
