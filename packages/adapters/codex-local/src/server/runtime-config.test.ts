...
describe("prepareCodexRuntimeConfig", () => {
  it("merges the default approved MCP server when PAPERCLIP_CODEX_PROVIDERS is unset", async () => {
    ...
    expect(prepared.notes.some((note) => note.includes("approved Codex MCP server"))).toBe(true);
    await prepared.cleanup();
    expect(await readConfigToml(home)).toBe("model = \"gpt-5.1-codex\"\n");
  });

  it("creates only managed MCP config for a fresh home and removes it on cleanup", async () => {
    const home = await makeCodexHome();
    const prepared = await prepareCodexRuntimeConfig({ env: {}, codexHome: home });

    expect(await readConfigToml(home)).toContain("[mcp_servers.playwright]");
    await prepared.cleanup();
    await expect(fs.access(path.join(home, "config.toml"))).rejects.toThrow();
  });

  it("does not write config when approved MCP sync and providers are disabled", async () => {
    const home = await makeCodexHome();
  ...
  it("surfaces a note for malformed PAPERCLIP_CODEX_PROVIDERS without touching config.toml", async () => {
    ...
    for (const [raw, noteFragment] of cases) {
      const prepared = await prepareCodexRuntimeConfig({
        env: { PAPERCLIP_CODEX_PROVIDERS: raw, PAPERCLIP_APPROVED_MCP_SERVERS: "none" },
        codexHome: home,
      });
  ...
  it("stays silent when PAPERCLIP_CODEX_PROVIDERS is unset or empty and approved MCP sync is disabled", async () => {
    const home = await makeCodexHome("model = \"gpt-5.1-codex\"\n");
    const envs: Array<Record<string, string>> = [
      { PAPERCLIP_APPROVED_MCP_SERVERS: "none" },
      { PAPERCLIP_CODEX_PROVIDERS: "", PAPERCLIP_APPROVED_MCP_SERVERS: "none" },
      { PAPERCLIP_CODEX_PROVIDERS: "  ", PAPERCLIP_APPROVED_MCP_SERVERS: "none" },
    ];
  ...
  it("restores user provider sections from the pre-run backup after an interrupted run", async () => {
    ...
    const prepared = await prepareCodexRuntimeConfig({ env: {}, codexHome: home });
    const content = await readConfigToml(home);
    expect(content).toContain('base_url = "http://user.example/v1"');
    expect(content).toContain("[mcp_servers.playwright]");
    await prepared.cleanup();
  ...
  it("uses adapter env PAPERCLIP_APPROVED_MCP_SERVERS before process env", async () => {
    const home = await makeCodexHome("model = \"gpt-5.1-codex\"\n");
    process.env.PAPERCLIP_APPROVED_MCP_SERVERS = "playwright";
    try {
      const prepared = await prepareCodexRuntimeConfig({
        env: { PAPERCLIP_APPROVED_MCP_SERVERS: "none" },
        codexHome: home,
      ...
      delete process.env.PAPERCLIP_APPROVED_MCP_SERVERS;
    }
  });

  it("removes stale managed MCP blocks when approved MCP sync is disabled", async () => {
    const home = await makeCodexHome();
    ...
    expect(prepared.notes.some((note) => note.includes("stale"))).toBe(true);
    const content = await readConfigToml(home);
    expect(content).not.toContain("[mcp_servers.playwright]");
    await prepared.cleanup();
  });

  it("wins over a pre-existing same-name [mcp_servers.*] section and restores it on cleanup", async () => {
    const original = [
    ...
    expect(content).toContain("[mcp_servers.playwright]");
    expect(content).toContain('command = "npx"');
    expect(content).not.toContain("old-playwright");
