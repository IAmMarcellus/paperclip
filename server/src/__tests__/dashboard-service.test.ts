...
describeEmbeddedPostgres("dashboard service", () => {
  ...
  it("buckets run activity by the requested timezone's calendar day", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();

    // 04:30 UTC, three days ago: still the *previous* calendar day in
    // America/Los_Angeles (UTC-7/-8), so the UTC and local buckets differ.
    const now = new Date();
    const instant = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - 3, 4, 30),
    );
    const utcKey = utcDateKey(instant);
    const laKey = new Intl.DateTimeFormat("en-CA", {
      timeZone: "America/Los_Angeles",
    }).format(instant);
    expect(laKey).not.toBe(utcKey); // sanity: the day boundary is actually crossed
    ...
    const bucketFor = (activity: { date: string; total: number }[], date: string) =>
      activity.find((bucket) => bucket.date === date)?.total ?? 0;

    const utcSummary = await dashboardService(db).summary(companyId);
    expect(bucketFor(utcSummary.runActivity, utcKey)).toBe(1);
    expect(bucketFor(utcSummary.runActivity, laKey)).toBe(0);

    const laSummary = await dashboardService(db).summary(companyId, {
      timeZone: "America/Los_Angeles",
    });
    expect(bucketFor(laSummary.runActivity, laKey)).toBe(1);
    expect(bucketFor(laSummary.runActivity, utcKey)).toBe(0);
  });
});
