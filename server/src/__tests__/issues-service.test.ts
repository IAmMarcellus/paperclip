...
describeEmbeddedPostgres("issueService.list commentedByUserId", () => {
  let db!: ReturnType<typeof createDb>;
  let svc!: ReturnType<typeof issueService>;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;
  ...
  it("returns only issues where the user authored a non-deleted comment", async () => {
    const companyId = randomUUID();
    const userId = "user-1";
    const otherUserId = "user-2";
    await db.insert(companies).values({
    ...
    const commentedIssueId = randomUUID();
    const otherCommentedIssueId = randomUUID();
    const deletedCommentIssueId = randomUUID();
    const untouchedIssueId = randomUUID();
    await db.insert(issues).values([
      { id: commentedIssueId, companyId, title: "I commented here", status: "todo", priority: "medium", createdByUserId: otherUserId, createdAt: new Date("2026-03-26T10:00:00.000Z"), updatedAt: new Date("2026-03-26T10:00:00.000Z") },
      { id: otherCommentedIssueId, companyId, title: "Only someone else commented", status: "todo", priority: "medium", createdByUserId: userId, createdAt: new Date("2026-03-26T10:00:00.000Z"), updatedAt: new Date("2026-03-26T10:00:00.000Z") },
      { id: deletedCommentIssueId, companyId, title: "My comment was deleted", status: "todo", priority: "medium", createdByUserId: otherUserId, createdAt: new Date("2026-03-26T10:00:00.000Z"), updatedAt: new Date("2026-03-26T10:00:00.000Z") },
      { id: untouchedIssueId, companyId, title: "I created but never commented", status: "todo", priority: "medium", createdByUserId: userId, assigneeUserId: userId, createdAt: new Date("2026-03-26T10:00:00.000Z"), updatedAt: new Date("2026-03-26T10:00:00.000Z") },
    ]);

    await db.insert(issueComments).values([
      { companyId, issueId: commentedIssueId, authorUserId: userId, body: "My comment", createdAt: new Date("2026-03-26T11:00:00.000Z"), updatedAt: new Date("2026-03-26T11:00:00.000Z") },
      { companyId, issueId: otherCommentedIssueId, authorUserId: otherUserId, body: "Not mine", createdAt: new Date("2026-03-26T11:00:00.000Z"), updatedAt: new Date("2026-03-26T11:00:00.000Z") },
      { companyId, issueId: deletedCommentIssueId, authorUserId: userId, body: "Since removed", createdAt: new Date("2026-03-26T11:00:00.000Z"), updatedAt: new Date("2026-03-26T11:00:00.000Z"), deletedAt: new Date("2026-03-26T12:00:00.000Z") },
    ]);

    const filtered = await svc.list(companyId, { commentedByUserId: userId });

    expect(filtered.map((issue) => issue.id)).toEqual([commentedIssueId]);
  });
});
