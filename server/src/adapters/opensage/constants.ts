...
/**
 * Default base URL of THIS Paperclip server, seeded into OpenSage session state so the agent's
 * `paperclip_*` tools can call back into the control plane. mergatriod runs Paperclip on :3100;
 * override via the adapter's `paperclipBaseUrl` config when bound elsewhere.
 */
export const DEFAULT_PAPERCLIP_BASE_URL = "http://localhost:3100";
