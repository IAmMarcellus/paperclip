...
0. **Bootstrap today's plan if missing (self-healing).** Check `$AGENT_HOME/memory/YYYY-MM-DD.md`
   for today's date. If the file is absent OR has no non-empty `## Today's Plan` section:
   - `mkdir -p "$AGENT_HOME/memory"`.
   - Load the standing roadmap (read-only): `GET /api/companies/{companyId}/goals` → the active
     company goal; its `description` holds the canonical roadmap when set. Fallbacks if empty: a
     `roadmap-30d` / `roadmap` document, else the latest planning / decision-log issue. Use your
     `AGENTS.md` priorities and validation criteria for ordering.
   - Pick the roadmap's current phase by today's date AND reconcile against what is already `done`
     on the board (do not re-plan finished work). Write `$AGENT_HOME/memory/YYYY-MM-DD.md`: a
     `# YYYY-MM-DD` header, a `## Today's Plan` of **2–5 concrete, delegable** next actions that
     advance the *already-approved* roadmap from its actual state (each names an owner agent and a
     checkable deliverable), then an empty `## Timeline`.
   - **Scope guard:** auto-plan execution/delegation only. New strategy, pricing, spend, or scope
     changes are NOT auto-planned — propose those to the board. If the roadmap is exhausted or
     unclear, the plan's single item is "post a board check-in proposing the next milestone," then exit.
   - Runs **only when the plan is missing** — never rewrite an existing plan on a heartbeat
     (token-efficiency: this fires at most once per day, when the date rolls over).
1. Read today's plan from `$AGENT_HOME/memory/YYYY-MM-DD.md` under "## Today's Plan".
2. Review each planned item: what's completed, what's blocked, and what up next.
3. For any blockers, resolve them yourself or escalate to the board.
4. If you're ahead, start on the next highest priority — **delegate it via section 6** (create child
   issues assigned to the owning agent, which wakes them; do not self-assign).
5. Record progress updates in the daily notes (`## Timeline`).
