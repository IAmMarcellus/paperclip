...
export function agentRoutes(
  ...
  // codex_local agents get an ISOLATED CODEX_HOME with OPENAI_API_KEY blanked (see the guard above),
  // so they authenticate from <CODEX_HOME>/auth.json. A freshly provisioned home has none and 401s on
  // first run — including agents a CEO hires autonomously. Seed it at create time by symlinking the
  // shared codex login in (CODEX_SHARED_AUTH_JSON, default ~/.codex/auth.json). Best-effort and
  // idempotent: it NEVER throws, so a seeding failure can't block agent creation; if the shared auth
  // isn't present yet it's a no-op. Sharing the whole CODEX_HOME is forbidden (assertCodexLocalHome-
  // IsNotShared) — we share only the credential file.
  function seedCodexLocalAuthForNewAgent(agent: {
    id: string;
    companyId: string;
    adapterType: string;
    ...
    try {
      if (agent.adapterType !== "codex_local") return;
      const rawShared =
        asNonEmptyString(process.env.CODEX_SHARED_AUTH_JSON)
        ?? path.join(os.homedir(), ".codex", "auth.json");
      const sharedAuth = normalizeCodexLocalHomePath(rawShared);
      if (!fs.existsSync(sharedAuth)) return; // nothing to link yet (run `codex login` once)

      const env = asRecord(asRecord(agent.adapterConfig)?.env);
      const configuredHome = env ? asEnvBindingString(env.CODEX_HOME) : null;
      const codexHome = normalizeCodexLocalHomePath(
        configuredHome ?? codexLocalAgentHome(agent.companyId, agent.id),
      );
      ...
      try {
        ...
        if (
          st.isSymbolicLink()
          && fs.existsSync(authPath) // false for a dangling symlink
          && fs.realpathSync(authPath) === fs.realpathSync(sharedAuth)
        ) {
          needsLink = false;
        } else {
          fs.rmSync(authPath, { force: true }); // stale/wrong link or a real file
        }
      } catch {
        // authPath doesn't exist yet -> create it below
      }
      if (needsLink) {
        fs.mkdirSync(codexHome, { recursive: true });
        fs.symlinkSync(sharedAuth, authPath);
      }
    } catch (err) {
      console.warn(`[codex-auth] could not seed codex auth for agent ${agent.id}:`, err);
    }
  }
  ...
  router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
    ...
    seedCodexLocalAuthForNewAgent(agent);
