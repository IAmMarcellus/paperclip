...
export function companyRoutes(db: Db, storage?: StorageService) {
  ...
  router.post("/", validate(createCompanySchema), async (req, res) => {
    ...
    // Auto-provision a memory-mcp bearer token (shown once). Best-effort — never block company
    // creation on it; it can be re-minted via POST /:companyId/memory-mcp-token.
    let memoryMcpToken: string | undefined;
    try {
      ({ token: memoryMcpToken } = await mcpTokens.mintForCompany(company.id));
    } catch (err) {
      console.warn("memory-mcp token mint failed at company create", err);
    }
    res.status(201).json({ ...company, memoryMcpToken });
  });

  // Mint/rotate the company's memory-mcp token (also for companies created before this feature).
  router.post("/:companyId/memory-mcp-token", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    ...
    const { token } = await mcpTokens.mintForCompany(companyId);
    res.status(201).json({ memoryMcpToken: token });
  });
