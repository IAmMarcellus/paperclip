...
// Desktop browsers (Mozilla) and the Paperclip mobile app (CFNetwork) are the only
// clients that legitimately write to the API as the *implicit* local board — i.e. the
// human operator, who authenticates via a session cookie / trusted-local default and
// sends no Bearer token. Any other client (curl, python, node, …) writing as the bare
// local board is almost always a misauthenticated agent whose per-run PAPERCLIP_API_KEY
// never made it onto the request, so its write would be silently recorded as "Board".
const HUMAN_LOCAL_BOARD_UA_MARKERS = ["mozilla", "paperclip", "cfnetwork"];

function isUnauthenticatedNonBrowserBoardWrite(req: Request): boolean {
  if (req.actor?.type !== "board" || req.actor?.source !== "local_implicit") return false;
  const ua = (req.header("user-agent") ?? "").toLowerCase();
  return !HUMAN_LOCAL_BOARD_UA_MARKERS.some((marker) => ua.includes(marker));
}

// Enforcement is opt-in: rejecting these writes also blocks in-flight agent workflows
// until their auth is fixed, so we log by default and only 401 when explicitly enabled.
function shouldRejectUnauthenticatedLocalComments(): boolean {
  const flag = process.env.PAPERCLIP_REJECT_UNAUTHENTICATED_LOCAL_COMMENTS?.trim().toLowerCase();
...
export function issueRoutes(
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    const commentedByUserFilterRaw = req.query.commentedByUserId as string | undefined;
    ...
    const commentedByUserId =
      commentedByUserFilterRaw === "me" && req.actor.type === "board"
        ? req.actor.userId
        : commentedByUserFilterRaw;
    ...
    if (commentedByUserFilterRaw === "me" && (!commentedByUserId || req.actor.type !== "board")) {
      res.status(403).json({ error: "commentedByUserId=me requires board authentication" });
      return;
    }
  ...
  router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
    ...
    // Source-attribution guard: catch agent comment writes that arrived with no
    // credential and would otherwise be silently attributed to the local "Board"
    // (the diagnosed cause of agent posts showing up as the human operator). Always
    // surface it; only reject when PAPERCLIP_REJECT_UNAUTHENTICATED_LOCAL_COMMENTS is set.
    if (isUnauthenticatedNonBrowserBoardWrite(req)) {
      logger.warn(
        { issueId: id, userAgent: req.header("user-agent") ?? null, path: req.originalUrl },
        "comment write resolved to local-board with no credential from a non-browser client (likely a misauthenticated agent)",
      );
      if (shouldRejectUnauthenticatedLocalComments()) {
        res.status(401).json({
          error: "unauthenticated_agent_write",
          detail:
            "This comment was accepted as the local board but carried no agent credential, so it would be misattributed to the operator. Attach 'Authorization: Bearer $PAPERCLIP_API_KEY' and 'X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID' (see the paperclip skill). Unset PAPERCLIP_REJECT_UNAUTHENTICATED_LOCAL_COMMENTS to disable this check.",
        });
        return;
      }
    }
