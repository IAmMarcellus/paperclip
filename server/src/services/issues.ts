...
export interface IssueFilters {
  ...
  /** Match issues on which this user has authored at least one (non-deleted) comment. */
...
function commentedByUserCondition(companyId: string, userId: string) {
  return sql<boolean>`
    EXISTS (
      SELECT 1
      FROM ${issueComments}
  ...
  return {
    ...
    list: async (companyId: string, filters?: IssueFilters) => {
      ...
      const offset = typeof filters?.offset === "number" && Number.isFinite(filters.offset)
        ...
        : 0;
      const touchedByUserId = filters?.touchedByUserId?.trim() || undefined;
      const commentedByUserId = filters?.commentedByUserId?.trim() || undefined;
      const inboxArchivedByUserId = filters?.inboxArchivedByUserId?.trim() || undefined;
      const unreadForUserId = filters?.unreadForUserId?.trim() || undefined;
      const contextUserId = unreadForUserId ?? touchedByUserId ?? inboxArchivedByUserId;
      ...
      if (commentedByUserId) {
        conditions.push(commentedByUserCondition(companyId, commentedByUserId));
      }
    ...
    count: async (companyId: string, filters?: IssueFilters) => {
      ...
      if (filters?.assigneeUserId) conditions.push(eq(issues.assigneeUserId, filters.assigneeUserId));
      const countCommentedByUserId = filters?.commentedByUserId?.trim() || undefined;
      if (countCommentedByUserId) {
        conditions.push(commentedByUserCondition(companyId, countCommentedByUserId));
      }
      if (filters?.projectId) conditions.push(eq(issues.projectId, filters.projectId));
      if (filters?.workspaceId) {
        conditions.push(or(
