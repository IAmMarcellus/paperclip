...
Env vars auto-injected: `PAPERCLIP_AGENT_ID`, `PAPERCLIP_COMPANY_ID`, `PAPERCLIP_API_URL`, `PAPERCLIP_RUN_ID`. Optional wake-context vars may also be present: `PAPERCLIP_TASK_ID` (issue/task that triggered this wake), `PAPERCLIP_WAKE_REASON` (why this run was triggered), `PAPERCLIP_WAKE_COMMENT_ID` (specific comment that triggered this wake), `PAPERCLIP_APPROVAL_ID`, `PAPERCLIP_APPROVAL_STATUS`, and `PAPERCLIP_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `PAPERCLIP_API_KEY` is auto-injected as a short-lived run JWT. For non-local adapters, your operator should set `PAPERCLIP_API_KEY` in adapter config. All endpoints are under `/api` and speak JSON.

**Call the API through the `paperclip-api` wrapper — always.** It lives at `scripts/paperclip-api.sh` in this skill and automatically attaches `Authorization: Bearer $PAPERCLIP_API_KEY` and `X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID`, so every request is attributed to **you** and linked to your run. Hand-writing `curl` is error-prone: if you forget those headers, the write is silently accepted as the operator ("Board") instead of you. The wrapper also fails loudly if the credential is missing, rather than sending a bare (unauthenticated) request.

```
scripts/paperclip-api.sh <METHOD> <path> [body]      # <path> is relative to /api; the leading /api is optional

scripts/paperclip-api.sh GET   /agents/me
scripts/paperclip-api.sh POST  /issues/{issueId}/comments '{"body":"…"}'
scripts/paperclip-api.sh PATCH /issues/{issueId} '{"status":"done"}'
printf '%s' "$body" | scripts/paperclip-api.sh POST /issues/{issueId}/comments -   # large body via stdin
scripts/paperclip-api.sh POST /issues/{issueId}/comments @/tmp/comment.json         # …or from a file
```

Never hard-code the host or the auth headers. Only fall back to raw `curl` if the wrapper is genuinely unavailable — and then you MUST include both `-H "Authorization: Bearer $PAPERCLIP_API_KEY"` and `-H "X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID"` (double quotes, so they expand).
...
**Run audit trail:** The `paperclip-api` wrapper attaches `X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID` to every request, linking your actions to the current heartbeat run. If you ever fall back to raw `curl`, include `-H "X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID"` — **double quotes so it expands** — on all issue-modifying requests (checkout, update, comment, create subtask, release).
...
```
scripts/paperclip-api.sh POST /issues/{issueId}/checkout \
...
```bash
scripts/paperclip-api.sh PATCH /issues/{issueId} '{ "status": "done", "comment": "What was done and why." }'
```
...
```bash
# Build the JSON with jq so literal newlines survive, then pipe it to the wrapper:
jq -Rs '{ status: "done", comment: . }' <<'MD' | scripts/paperclip-api.sh PATCH /issues/"$PAPERCLIP_TASK_ID" -
Done
