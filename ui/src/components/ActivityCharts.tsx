...
// Calendar date (YYYY-MM-DD) for `date` in the browser's local timezone — NOT
// UTC. `toISOString` would roll an evening west-of-UTC into tomorrow's bucket.
...
export function getLast14Days(): string[] {
  return Array.from({ length: 14 }, (_, i) => {
    const d = new Date();
    d.setHours(12, 0, 0, 0); // anchor at local noon so DST shifts can't slip the day
...
function aggregateRuns(runs: readonly HeartbeatRun[] = []): DashboardRunActivityDay[] {
  ...
  for (const run of runs) {
    const day = localDayKey(new Date(run.createdAt));
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  for (const issue of issues) {
    const day = localDayKey(new Date(issue.createdAt));
    const entry = grouped.get(day);
    if (!entry) continue;
