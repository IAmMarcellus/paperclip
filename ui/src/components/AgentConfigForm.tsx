...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  // Mirror current dirtiness into a ref so the agent-refresh effect below (which intentionally does
  // not depend on `overlay`) can read it without a stale closure.
  const isDirtyRef = useRef(false);

  // When the agent record refreshes, drop the overlay so the form reflects the latest saved state —
  // but NEVER clobber an in-progress (dirty) edit. A running agent's record changes on every
  // background refetch / window-focus refetch; without this guard those refreshes silently discarded
  // the user's unsaved changes before they could Save (e.g. switching an agent's adapter type, which
  // is why that edit "never took effect"). A successful save clears the overlay explicitly in
  // handleSave, so the post-save reset still happens.
  useEffect(() => {
    if (!isCreate) {
      if (
  ...
  const handleSave = useCallback(async () => {
    ...
    // Save succeeded — clear the overlay so the form reflects the persisted state. (On failure the
    // await throws and we keep the overlay so the edit can be retried.) The agent-refresh effect no
    // longer clears it, because it must preserve dirty edits across background refetches.
    setOverlay({ ...emptyOverlay });
