...
interface IssuesListProps {
  ...
  /**
   * Enables the "Commented on by me" filter toggle in the filter popover.
   * Only pass this where the parent threads the resulting flag into its issue
   * fetch (via {@link onCommentedByMeChange}); the filter is server-backed.
   */
  enableCommentedByMeFilter?: boolean;
  ...
  /** Reports the current "commented on by me" toggle so the parent can filter its fetch server-side. */
  onCommentedByMeChange?: (value: boolean) => void;
...
export function IssuesList({
  ...
  enableCommentedByMeFilter = false,
  hasMoreIssues = false,
  ...
  // The "commented on by me" filter is server-backed, so report the toggle up to
  // the parent (which owns the fetch) rather than filtering the loaded list here.
  const commentedByMe = viewState.commentedByMe === true;
  useEffect(() => {
    onCommentedByMeChange?.(commentedByMe);
  }, [commentedByMe, onCommentedByMeChange]);
  ...
  const activeFilterCount = countActiveIssueFilters(viewState, enableRoutineVisibilityFilter, enableCommentedByMeFilter);
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <IssueFiltersPopover
            ...
            enableCommentedByMeFilter={enableCommentedByMeFilter}
            iconOnly
