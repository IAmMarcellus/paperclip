  ...
  return (
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 pointer-coarse:gap-3 px-3 py-2">
        {/* <REDACTED_ORG> — the umbrella over every company. Pinned above the per-company nav, it
            opens the instance-wide HQ (cross-company overview + the Mergatroid Conference Room).
            Same experimental gate as the per-company Conference Room. */}
        {conferenceRoomChatEnabled ? (
          <div className="flex flex-col gap-0.5 border-b border-border pb-3">
            <SidebarNavItem to="/hq" label="<REDACTED_ORG>" icon={Building2} end />
          </div>
        ) : null}
