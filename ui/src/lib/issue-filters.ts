...
export type IssueFilterState = {
  ...
  liveOnly?: boolean;
  /**
   * Restrict to tasks the current user has commented on. Applied server-side
   * (comment authorship isn't part of the loaded issue payload), so this flag
   * is threaded into the fetch query rather than into {@link applyIssueFilters}.
   */
...
export function normalizeIssueFilterState(value: unknown): IssueFilterState {
  ...
  return {
    ...
    commentedByMe: candidate.commentedByMe === true,
...
export function countActiveIssueFilters(
  ...
  if (enableCommentedByMeFilter && state.commentedByMe) count += 1;
