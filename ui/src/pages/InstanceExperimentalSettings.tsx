...
export function InstanceExperimentalSettings() {
  ...
  const enableVoiceChat = experimentalQuery.data?.enableVoiceChat === true;
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">Voice (Mergatroid)</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              Adds a hands-free “Call” tab to <REDACTED_ORG> HQ so you can talk to Mergatroid via
              ElevenLabs. Requires the Conference Room / HQ surface, the ElevenLabs server config
              (ELEVENLABS_API_KEY + ELEVENLABS_AGENT_ID), and the public LLM-shim tunnel — see
              docs/elevenlabs-voice.md.
            </p>
          </div>
          <ToggleSwitch
            checked={enableVoiceChat}
            onCheckedChange={() => toggleMutation.mutate({ enableVoiceChat: !enableVoiceChat })}
            disabled={toggleMutation.isPending}
            aria-label="Toggle Mergatroid voice experimental setting"
          />
        </div>
      </section>
