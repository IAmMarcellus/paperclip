...
export function Issues() {
  ...
  // Server-backed "commented on by me" filter, lifted from the filter popover
  // inside IssuesList so it can scope the paginated fetch below.
  const [commentedByMe, setCommentedByMe] = useState(false);
  ...
  const {
    ...
    queryFn: ({ pageParam }) => issuesApi.list(selectedCompanyId!, {
      ...
      ...(commentedByMe ? { commentedByUserId: "me" } : {}),
  ...
  return (
    <IssuesList
      ...
      enableCommentedByMeFilter
      onCommentedByMeChange={setCommentedByMe}
      hasMoreIssues={hasMoreServerIssues}
      onLoadMoreIssues={loadMoreServerIssues}
      onUpdateIssue={(id, data) => updateIssue.mutate({ id, data })}
      searchFilters={
        participantAgentId || workspaceIdFilter || commentedByMe
          ? {
              participantAgentId,
              workspaceId: workspaceIdFilter,
              ...(commentedByMe ? { commentedByUserId: "me" } : {}),
